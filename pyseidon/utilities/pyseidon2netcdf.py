...
def pyseidon_to_netcdf(fvcom, filename, debug):
    ...
    for var in varname:
        if var in ['ua', 'va','depth_av_flow_dir','depth_av_vorticity',
                   ...
                   'hori_velo_norm']:
            try:
                globals()[var] = f.createVariable(var, 'float', ('time','nele'))
                globals()[var][:] = getattr(fvcom.Variables, var)[:]
            except AttributeError:
                pass
        if var in ['julianTime', 'matlabTime']:
            try:
                globals()[var] = f.createVariable(var, 'float', ('time',))
                globals()[var][:] = getattr(fvcom.Variables, var)[:]
            except AttributeError:
                pass
        if var == 'el':
            try:
                globals()[var] = f.createVariable(var, 'float', ('time','node'))
                globals()[var][:] = getattr(fvcom.Variables, var)[:]
            except AttributeError:
                pass
        if fvcom.Variables._3D:
            if var in ['u', 'v', 'flow_dir', 'velo_norm',
                       'vorticity', 'power_density']:
                try:
                    globals()[var] = f.createVariable(var,'float',('time','siglay','nele'))
                    globals()[var][:] = getattr(fvcom.Variables, var)[:]
                except AttributeError:
                    pass
            if var in ['verti_shear']:
                try:
                    globals()[var] = f.createVariable(var,'float',
                                                      ('time','vertshear','nele'))
                    globals()[var][:] = getattr(fvcom.Variables, var)[:]
                except AttributeError:
                    pass
    ...
    for grd in gridname:
        if grd in ['xc', 'yc', 'lonc', 'latc', 'hc']:
            try:
                globals()[grd] = f.createVariable(grd, 'float', ('nele',))
                globals()[grd][:] = getattr(fvcom.Variables, grd)[:]
            except AttributeError:
                pass
        if grd == 'depth2D':
            try:
                depth2D = f.createVariable(grd, 'float', ('time','nele'))
                depth2D[:] = getattr(fvcom.Variables, grd)[:]
            except AttributeError:
                pass
        if grd in ['x', 'y', 'lon', 'lat', 'h']:
            try:
                globals()[grd] = f.createVariable(grd, 'float', ('node',))
                globals()[grd][:] = getattr(fvcom.Variables, grd)[:]
            except AttributeError:
                pass
        if grd in ['triele']:
            try:
                globals()[grd] = f.createVariable(grd, 'i', ('nele','three'))
                globals()[grd][:] = getattr(fvcom.Variables, grd)[:]
            except AttributeError:
                pass
        if grd in ['trinodes']:
            try:
                globals()[grd] = f.createVariable(grd, 'i', ('node','three'))
                globals()[grd][:] = getattr(fvcom.Variables, grd)[:]
            except AttributeError:
                pass
        if grd in ['a1u', 'a2u']:
            try:
                globals()[grd] = f.createVariable(grd, 'i', ('four','nele'))
                globals()[grd][:] = getattr(fvcom.Variables, grd)[:]
            except AttributeError:
                pass
        if grd in ['aw0', 'awy', 'awx']:
            try:
                globals()[grd] = f.createVariable(grd, 'i', ('three','nele'))
                globals()[grd][:] = getattr(fvcom.Variables, grd)[:]
            except AttributeError:
                pass
        if fvcom.Variables._3D:
            if grd == 'siglay':
                try:
                    ...
                    siglay[:] = getattr(fvcom.Variables, grd)[:]
                except AttributeError:
                    pass
            if grd == 'siglev':
                try:
                    siglev = f.createVariable(grd,'float', ('siglev','node'))
                    siglev[:] = getattr(fvcom.Variables, grd)[:]
                except AttributeError:
                    pass
            if grd == 'depth':
                try:
                    depth = f.createVariable(grd,'float', ('time','siglay','nele'))
                    depth[:] = getattr(fvcom.Variables, grd)[:]
                except AttributeError:
                    pass
    if debug: print "...done"
    f.close()

#Note el should be change to zeta; julianTimes to Times; nnode to node; trinodes to nv; triele to nbe; add 1 to nv nbe      
